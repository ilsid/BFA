...
public class PrepaidCheckBalanceAndReserve extends Script {
	...
	protected void doExecute() throws ScriptException {
		...
		DeclareLocalVar("GlobalConfiguration", "DefaultConfiguration", GetGlobalVar("GlobalConfiguration"));
		DeclareLocalVar("Cents", "Decimal", 100.0);
		...
		if (EqualCondition("GlobalConfiguration.EnableUnipierPrepaidAccounting", AsBoolean("True"), 
			"Manage Subscriber Prepaid Account?")) {
			
			if (LessOrEqualCondition("Amount", "Subscriber.PrepaidAmount - Subscriber.PrepaidReserved", 
				"Is there enough to cover the charge for subscription?")) {
			
				if (EqualCondition("GlobalConfiguration.isICMActive", AsBoolean("True"), "ICM activated?")) {
					SetGlobalVar("PrPResponded", AsBoolean("True"));
					Action("Round", "TempAmount * Cents").SetLocalVar("AmountToReserve");
					
					Action("Reserve Amount", 
							"Subscriber.msisdn", 
							"Subscriber.IMSI", 
							"Subscriber.SCPAddress", 
							"AmountToReserve", 
							AsString("EVENT_BASED"), 
							"AccessType")
